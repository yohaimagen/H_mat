"""Tests for box-adjacency and neighbor lists `L^nei` (Task 1.4, revised by
Task F.2 to a flat, node-keyed map).
...
def test_neighbor_lists_2d_interior_has_9() -> None:
    ...
    for node in level_nodes:
        ...
        if not is_boundary:
            found_interior = True
            assert len(nei[node]) == 9, f"interior box has {len(nei[node])} neighbors"
...
def test_neighbor_lists_2d_corner_has_4() -> None:
    ...
    for node in level_nodes:
        ...
        if n_boundary_axes == 2:
            ...
            assert len(nei[node]) == 4, f"corner box has {len(nei[node])} neighbors"
...
def test_neighbor_lists_3d_interior_has_27() -> None:
    ...
    for node in level_nodes:
        ...
        if not is_boundary:
            found_interior = True
            assert len(nei[node]) == 27, f"interior box has {len(nei[node])} neighbors"
...
def test_neighbor_lists_1d_equivalent_interior_has_3() -> None:
    ...
    for node in level_nodes:
        ...
        if not is_boundary:
            found_interior = True
            assert len(nei[node]) == 3, f"interior box has {len(nei[node])} neighbors"
...
def test_box_includes_itself() -> None:
    ...
    root = build_tree(mesh, m=2)
    nei = neighbor_lists(root)
    for node, neighbors in nei.items():
        assert any(node is b for b in neighbors), f"node {node} does not include itself"
...
def test_neighbor_relation_is_symmetric_2d() -> None:
    ...
    root = build_tree(mesh, m=2)
    nei = neighbor_lists(root)
    for alpha, neighbors in nei.items():
        for beta in neighbors:
            assert alpha in nei[beta], f"neighbor relation not symmetric between {alpha} and {beta}"
...
def test_boundary_box_has_fewer_neighbors_than_interior_3d() -> None:
    ...
    for node in level_nodes:
        ...
        if is_boundary:
            counts_boundary.append(len(nei[node]))
        else:
            counts_interior.append(len(nei[node]))
